...
class PineconeStore extends base_js_1.VectorStore {
    ...
    async addVectors(vectors, documents, options) {
        ...
        //     },
        // })));
        const upsertNamespace = this.pineconeIndex.namespace(this.namespace);
        const batchRequests = chunkedVectors.map((chunk) => this.caller.call(async () => {
            console.log(chunk);
            await upsertNamespace.upsert(chunk)
        }
        ));
    ...
    async similaritySearchVectorWithScore(query, k, filter) {
        ...
        const queryNamespace = this.pineconeIndex.namespace(this.namespace);
        const results = await queryNamespace.query({
            // queryRequest: {
